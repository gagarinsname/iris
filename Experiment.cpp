...
)
{
	...
	for (i = 0; i < imgEnd; ++i)
	{
		...
		printf("Filename: %s", cMarkupData.filename);
		...
		printf("error: pCenter = %f, iCenter = %f.\n", error.pCenter, error.iCenter);
		fprintf(fout, "%f %f\n", error.pCenter, error.iCenter);

		if (error.iCenter > 0.3)
		{
			DRAW_CircleInGray(imageData, W, H, Result.IrisData->sCircle, 255);
			if ((res = SaveBmp8(imageOutputPath, "_c.bmp", W, H, imageData, 4)) != 0)
			{
				printf("ERROR: Saving segmentation result failed.");
			}
		}
		...
		fclose(imageFile);
		//printf("File: %s, Pupil: (%d,%d,%d).\n", cMarkupData.filename, cMarkupData.pupil.xc, cMarkupData.pupil.yc, cMarkupData.pupil.r);
	}
...
int main(int argc, char** argv)
{
	// load markup file
	int res;
	FILE* fout = fopen("D:/data/res/0224/log.txt", "wb");
	if ((res = processMarkup(0, 2325, fout)) != 0)
	{
